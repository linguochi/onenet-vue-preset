...
// 会在项目根目录生成 vue-pages.config.js 文件，用作整个项目的配置文件。
// 但是 如果在template中存在 vue-pages.config.js 配置文件，render后的项目的 (和 package.json 同级的) 根目录中存在这个文件，那么它会被 @vue/cli-service 自动加载.
...
const generatePagesConfig = ({ pagesDir, customConfig = {} }) => {
  ...
  // 读取多页面目录生成 vue-pages.config.js 中的 pages 配置
  if (!fs.existsSync(PAGES_DIR)) {
    throw Error('vue-pages.config.js pages 配置路径不存在');
  }
